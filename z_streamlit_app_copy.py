...
def interact_with_gpt(sys_prompt, user_prompt, file_content, gpt_model, client):
    question_prompt = "Je vous ai envoyé un document, l'avez-vous reçu?"
...
def gpt_txt_file(txt_file_path, sys_prompt, user_prompt, gpt_model, client, output_csv_file_name="txt"):
    file_content = read_text_file(txt_file_path)
    if not file_content:
        return "Erreur lors de la lecture du fichier."

    completion = interact_with_gpt(sys_prompt, user_prompt, file_content, gpt_model, client)
    if not completion:
